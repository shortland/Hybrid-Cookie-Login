...
use String::Random;
use Cwd qw(abs_path);
use HybridAuth::Regex qw(isCharNum);
use Exporter qw(import);
use Data::Dumper;
our @EXPORT_OK = qw(getConfig userExists createDBH isNotValidDetails hashPassword isUserPasswordValid getUserHash setCookiesEIE makeNewCookies updateDBCookies);

sub getConfig {
    return LoadFile(abs_path . '/HybridAuth/.config.yaml');
}

sub getUserIDN {
    my ($u, $DBH) = @_;
    my $table = getConfig->{sql}{table};
    my $sth = $DBH->prepare("SELECT IDN FROM $table WHERE username = ?");
...
sub updateDBCookies {
    my ($u, $uC, $pC, $DBH) = @_;
    my $table = getConfig->{sql}{table};
    my $sth = $DBH->prepare("UPDATE $table SET u_cookie = ?, p_cookie = ? WHERE username = ?");
...
sub createDBH {
    ...
    return DBI->connect("DBI:mysql:database=" . $DATABASE . ";host=" . $config->{sql}{host}, $config->{sql}{user}, $config->{sql}{pass}, {'RaiseError' => 1});
}
...
sub userExists {
    my ($u, $DBH) = @_;
    my $table = getConfig->{sql}{table};
    my $sth = $DBH->prepare("SELECT hash FROM $table WHERE username = ?");
    $sth->execute($u) or die "Couldn't execute statement: $DBI::errstr; stopped";
